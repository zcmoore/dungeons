...
/**
 * Represents a type of dice (e.g. a 20-sided dice, or a 10-sided dice).
 * This class contains specifics about the dice, and calculates the rolls thereof.
 * 
 ...
 * 
 * All Dice objects are mapped upon instantiation. Additionally, any two
 * Dice objects that hold the same values will reference the same
 * location in memory.
 * 
...
public class Dice 
{
	...
	private int numberingOffset; 	//e.g. a 10-sided dice starts its numbering 
	...
	private Dice(int numberOfSides, int numberingOffset) 
	{
		this.numberOfSides = numberOfSides;
		this.numberingOffset = numberingOffset;
		this.name = generateName(numberOfSides);
	}
	
	/**
	 * Allows the instantiation of Dice objects outside of this class.
	 * New Dice will be mapped by this method.
	 * 
	 * @param numberOfSides		numberOfSides the dice has
	 * @param numberingOffset	e.g. a 10-sided dice starts its numbering 
	 * 							at 0, so the offset is 0. A d20 starts at 1, 
	 * 							so offset = 1, etc.
	...
	public static Dice constructDice(int numberOfSides, int numberingOffset)
	{
		...
		else
		{
			...
			returnDice = diceMap.get(generateName(numberOfSides));
		}
		...
		if (returnDice == null)
		{
			...
			diceMap.put(returnDice.name, returnDice);
		}
	...
	public static Dice parseDice(String dice)
	{
		...
		try {defaultDice = DefaultDice.valueOf(diceName);}
		catch (Exception e) {defaultDice = null;}
		...
		try
		{
			...
			else //dice is a default dice
			{
				...
				returnDice = constructDice(defaultDice.numberOfSides, 
